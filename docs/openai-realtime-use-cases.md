---
title: "Use Case Document: OpenAI Realtime Voice Provider Support"
status: draft
created: 2026-04-02
---
...
Reads the `provider` field. If `provider = 'phonic'`, constructs and returns a `phonic.realtime.RealtimeModel` with `voice` set to `externalId`. If `provider = 'openai'`, constructs and returns an `openai.realtime.RealtimeModel` with `voice` set to `externalId`. Both models implement `llm.RealtimeModel` from `@livekit/agents`.

Returns a configured `llm.RealtimeModel` instance.

Failure cases:
- If `provider` is neither `'phonic'` nor `'openai'`, throws `Error('Unsupported voice provider: {provider}')`
...
| ID | Rule |
|---|---|
| BR-01 | The `provider` field of the resolved voice row determines which plugin class (`phonic.realtime.RealtimeModel` or `openai.realtime.RealtimeModel`) is instantiated. |
...
|---|---|---|---|
| `voices.provider` | varchar(255) | not null, one of: `'phonic'`, `'openai'` | Determines which LiveKit agents plugin handles the session. |
...
| `DEFAULT_VOICE_PROVIDER` | env string | one of: `'phonic'`, `'openai'`, default: `'phonic'` | Controls which provider is used when a company has no voice configured. |
