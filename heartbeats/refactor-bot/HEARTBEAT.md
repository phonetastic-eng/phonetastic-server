...
## Step 1: Find unstarted refactor issues
...
For each refactor issue, use the Agent tool to spawn a sub-agent. Run up to 3 agents in parallel. Give each agent the following instructions (fill in the issue number and title):
...
- Preserve all existing behaviour — refactors must not change observable outcomes
...
```bash
git add -A
git commit -m "Refactor #ISSUE_NUMBER: [brief description]"
...
[1-3 bullet points describing the refactor]
